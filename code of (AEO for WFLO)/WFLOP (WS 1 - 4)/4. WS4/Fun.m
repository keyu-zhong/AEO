function [cost_per, P_sum, Efficiency, lp_power_accum, cr_position, xy_position] = Fun(pop, rows, cols, N, cell_width)
    theta = [0, pi/6, 2*pi/6, 3*pi/6, 4*pi/6, 5*pi/6, 6*pi/6, 7*pi/6, 8*pi/6, 9*pi/6, 10*pi/6, 11*pi/6];
    velocity = [13.0, 10.0, 7.0];
    f_theta_v = [0.058333, 0.016667, 0.008333].*ones(12,1);
    % fitness_val = 0;
    % P_sum = 0;
    % cost_per = 0;
