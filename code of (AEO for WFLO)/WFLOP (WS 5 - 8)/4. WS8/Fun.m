function [cost_per, P_sum, Efficiency, lp_power_accum, cr_position, xy_position] = Fun(pop, rows, cols, N, cell_width)
    theta = [0, pi / 6.0, 2 * pi / 6.0, 3 * pi / 6.0, 4 * pi / 6.0, 5 * pi / 6.0, 6 * pi / 6.0, ...
        7 * pi / 6.0, 8 * pi / 6.0, 9 * pi / 6.0, 10 * pi / 6.0, 11 * pi / 6.0]; % 风向角
    velocity = [12.0, 10.0, 8.0]; % 风速
    f_theta_v = [0.058333, 0.016667, 0.008333].*ones(12,1);% 风概率
    % fitness_val = 0;
    % P_sum = 0;
    % cost_per = 0;
